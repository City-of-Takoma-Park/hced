...
# custom packages - download with devtools::install_github("dpowerstp/packagename")
library(acsprocess)
library(tpfuncts)
library(leafletwrappers)
library(plotlywrappers)

# year working with - update this parameter
yd <- 2020
...
data_binder <- function(file_root, yeardownload = yd){
  # browser()
  acsprocess::data_binder(file_root = file_root, 
                          dir_root = glue::glue("./data/output_data/acs5_{yeardownload}/tpspec/"))
}
...
Please send any questions, comments, or feedback to Acting Planning Manager Alex Freedman at alexanderf@takomaparkmd.gov, Planning Intern Josh Cato at joshuac@takomaparkmd.gov, and Senior Policy and Data Analyst Dan Powers at danielp@takomaparkmd.gov.
...
pull_female_4049_tot <- pull_age_sex(c("female", "40-49"), "age_grp_est")
...
pull_male_4049_tot <- pull_age_sex(c("^male", "40-49"), "age_grp_est")
...
pull_diff_3039_tot <- (pull_female_3039_tot - pull_male_3039_tot)

pull_diff_4049_tot <- (pull_female_4049_tot - pull_male_4049_tot)
...
pull_tp_female_rel_pct <- pull_family_housppl(c("Takoma", "female", "relatives"))
pull_md_female_rel_pct <- pull_family_housppl(c("Maryland", "female", "relatives"))
pull_mc_female_rel_pct <- pull_family_housppl(c("Montgomery", "female", "relatives"))
...
# scotch-iish need be  excluded
pull_irish <- pull_ancestry_overall("^irish", "estimate")
pull_irish_pct <- pull_ancestry_overall("^irish", "pct_ancestry")
...
text_ancestry <- glue("**The Census reports {pull_num_ancestrygrps} ancestry groups in Takoma Park with at least 50 people, and no ancestry group represents more than {pull_ssa_pct}% of the population.** The largest reported group in Takoma Park by the Census's ancestry-categories are sub-Saharan African at { pull_ssa %>% commafy} (representing {pull_ssa_pct}% of residents), followed by Irish at {pull_irish %>% commafy} residents, German at {pull_german %>% commafy} residents, English at {pull_english %>% commafy} residents, and West Indian at {pull_westindian %>% commafy} residents. Due to the small numbers involved and high margin of error, caution should be taken in reading too much into exact estimates.")
...
pull_md_nocomp <- pull_comp(c("^Maryland", "No computer"), "pct_comp_int")

pull_mc_nocomp <- pull_comp(c("^Montgome", "No computer"), "pct_comp_int")
...
educ_asian <- educ_race_data_processed %>%
...
sub_race_compare_educ <- subplot(race_breakout_gen(educ_black, "Black",  TRUE), 
                                 ...
                                 race_breakout_gen(educ_asian, "Asian"), 
                                 shareX = TRUE, 
                                 shareY = TRUE,
                                 nrows = 3) %>%
...
pull_other_nohealth_md <- pull_race_noinsur(c("Maryland", "other"), "pct_health")
...
- Both Hispanic and Black residents also have less access to health insurance than white residents at statistically-significant levels. {pull_hisp_nohealth}% of Hispanic residents do not have health insurance ({pull_hisp_nohealth_tot %>% commafy} total), and {pull_black_nohealth}% of Black residents do not have health insurance ({pull_black_nohealth_tot %>% commafy} total).")

text_health_race_noinsur_compare <- glue("**Gaps for Black residents in Takoma Park exceed gaps in Montgomery County and Maryland at statistically significant levels, as do gaps for residents with a race of 'other.'** In Montgomery County and Maryland, {pull_black_nohealth_mont}% and {pull_black_nohealth_md}% of Black residents don't have health insurance, compared to {pull_black_nohealth}% of Black Takoma Park residents. {pull_other_nohealth}% of residents with a race of 'other' in Takoma Park do not have health insurance, compared to {pull_other_nohealth_mont}% of Montgomery County residents with a race of 'other' and {pull_other_nohealth_md}% in Maryland.")
...
text_race_age_health_summary <- glue("**Disparities in health-insurance access appear between white residents and most other races in the 19-64 year old age-range.** Between 19 and 64, only {pull_health_1964_white}% of white residents lack health insurance, compared to {pull_health_1964_other}% of residents with a race of 'other', {pull_health_1964_hisp}% of Hispanic residents, {pull_health_1964_black}% of Black residents, and {pull_health_1964_asian}% of Asian residents. Black residents and residents with a race of 'other' between 19 and 64 in Takoma Park also have less access to health insurance than residents of the same race in Montgomery County and Maryland.")

text_race_age_health <- glue("Low estimate-totals make assessing the statistical significance of disparities in health insurance access by age and race difficult. **Nonetheless, statistically significant disparities in access appear between white and people of color residents in the 19-64 year old age-range.**
...
tenure_age <- tenure_age_root %>%
  ...
  rename(pct_age_old_moe = pct_moe) %>%
...
renter_costs <- read_rds(
  glue::glue("./data/output_data/acs5_{yd}/tpspec/household_income_rentercosts_processed_tp.rds")
)

owner_costs <- read_rds(
  glue::glue("./data/output_data/acs5_{yd}/tpspec//household_income_ownercosts_processed_tp.rds")
)
...
pull_asian_income_household_less35 <- pull_race_income_household_less35("Asian", "percent_race_households") %>%
  sum
...
pull_asian_income_household_more100 <- pull_race_income_household_more100("asian", "percent_race_households") %>%
  sum
...
text_incomedist_race <- glue("**Reviewing more granular household income data provides more insight into these disparities.** {pull_asian_income_household_less35}% of Takoma Park's Asian households, {pull_black_income_household_less35}% of its Black households, and {pull_hispanic_income_household_less35}% of its Hispanic households earn less than $35,000 a year, compared to {pull_white_income_household_less35}% of its white households By contrast, {pull_white_income_household_more100}% of white households earned more than $100,000 a year, compared to {pull_black_income_household_more100}% of Black households and {pull_hispanic_income_household_more100}% of Hispanic households; and {pull_white_income_household_more200}% of its white households earn more than $200,000, compared to {pull_black_income_household_more200}% of Black and {pull_hispanic_income_household_more200}% of Hispanic households. Income-disparities exist among Asian residents in Takoma Park (although the low number of residents and high margin of error suggest caution); {pull_asian_income_household_less35}% of Asian households earn less than $35,000 annually, but {pull_asian_income_household_more100}% earn more than $100,000. Although the low number of residents with a race of 'other' should caution over-interpreting the data, these residents show similar levels of income disparities to Black residents.")
...
pull_mont_black_incomeless35 <- pull_place_race_incomeless35(c("Montgomery", "Black"), "percent_race_households") %>%
  sum
pull_mont_asian_incomeless35 <- pull_place_race_incomeless35(c("Montgomery", "Asian"), "percent_race_households") %>%
  sum
pull_md_asian_incomeless35 <- pull_place_race_incomeless35(c("^Maryland$", "Asian"), "percent_race_households") %>%
  sum


pull_ratio_tpmont_asian_incomeless35 <- (pull_asian_income_household_less35 / pull_mont_asian_incomeless35) %>% round(2)

pull_ratio_tpmd_asian_incomeless35 <- (pull_asian_income_household_less35 / pull_md_asian_incomeless35) %>% round(2)
...
race_income_household_asian <- race_income_household_data %>%
  filter(grepl("Asian", race))
...
plot_raceincomecompare <- subplot(race_breakout_gen(race_income_household_black, "Black", TRUE, x = "income_recode"), 
                                  ...
                                  race_breakout_gen(race_income_household_asian, "Asian", x = "income_recode"),
                                  shareX = TRUE, 
                                  shareY = TRUE,
                                  nrows = 3) %>%
...
pull_fem_pov_mont_pct <- pull_houstyp_pov(c("Mont", "Female nonfamily"))

pull_fem_pov_md_pct <- pull_houstyp_pov(c("^Maryland", "Female nonfamily"))


pull_male_pov_tp_pct <- pull_houstyp_pov(c("Takoma", "^Male nonfamily"))

pull_male_pov_mont_pct <- pull_houstyp_pov(c("Mont", "^Male nonfamily"))

pull_male_pov_md_pct <- pull_houstyp_pov(c("Maryland", "^Male nonfamily"))


pull_marr_pov_tp_pct <- pull_houstyp_pov(c("Takoma", "Married"))
pull_femfam_pov_tp_pct <- pull_houstyp_pov(c("Takoma", "Single female"))

pull_femfam_pov_md <- pull_houstyp_pov(c("Maryland", "Single female"))
...
pull_marr_nochild_pov_tot <- pull_family_pov(c("Takoma", "Married: no chi"), "pov_fam_est")
...
pull_pov_tp_female_asian <- pull_pov_sex_race(c("Takoma", "Female", "asian"), 
                                                  "pct_race_sex_pov")

pull_pov_md_female_asian <- pull_pov_sex_race(c("^Maryland", "Female", "asian"), 
                                                  "pct_race_sex_pov")

pull_pov_mont_female_asian <- pull_pov_sex_race(c("Mont", "Female", "asian"), 
                                                  "pct_race_sex_pov")
...
pull_tp_poverty_asian_pctall <- pull_poverty_race(c("Takoma", "asian"), "pct_all")
...
pull_md_poverty_asian <- pull_poverty_race(c("^Maryland", "asian"), "pct_poverty")

pull_mc_poverty_asian <- pull_poverty_race(c("Montgomery", "asian"), "pct_poverty")


#
text_poverty_race <- glue("**There are significant racial disparities in poverty rates.** The most residents in poverty by race are Black (representing {pull_tp_poverty_black_tot} total residents), followed by white ({pull_tp_poverty_white_tot}) and Hispanic ({pull_tp_poverty_hispanic_tot}) residents. As discussed previously, the official poverty line undercounts the number of residents actually experiencing poverty, but the Census does not make race-disaggregated data available on residents' income relative to the poverty line. The counts of residents in poverty discussed here undercount the number of residents actually experiencing poverty, even if we cannot know the exact count.

**Shares of persons in poverty differ from population shares.** Black residents represent {pull_black_tp_pct}% of the city's population but {pull_tp_poverty_black_pctall}% of residents in poverty, and their poverty rate of {pull_tp_poverty_black}% has a statistically significant difference from the city's rate of {pull_tp_poverty_overall}%. Asian residents make up {pull_asian_tp_pct}% of the population but {pull_tp_poverty_asian_pctall}% of residents in poverty, with the City's highest poverty rate of {pull_tp_poverty_asian}% that's significantly higher than both the overall poverty line and white residents. White residents make up {pull_white_tp_pct}% of the city's population but just {pull_tp_poverty_white_pctall}% of those in poverty, with a statistically-lower poverty rate of {pull_tp_poverty_white}%.

**Residents of multiple races have a statistically-higher poverty rate than white residents at a 90% level of confidence,** including Black residents and Asian residents ({pull_tp_poverty_asian}%). Residents with a race of 'other' in the data have the highest estimate of their poverty rate at {pull_tp_poverty_other}%, but the high margin of error of {pull_tp_poverty_other_moe}% means our estimate of these residents' actual poverty rate at a 90% confidence level falls between {round(pull_tp_poverty_other - pull_tp_poverty_other_moe, 1)}% and {pull_tp_poverty_other + pull_tp_poverty_other_moe}%.

There is not a statistically significant difference between poverty rates of different races in Takoma Park vs Maryland or Montgomery County, except that white residents have a lower poverty rate of {pull_tp_poverty_white}% than Maryland ({pull_md_poverty_white}%), and Asian residents have a higher poverty rate than Maryland ({pull_md_poverty_asian}%) or Montgomery County ({pull_mc_poverty_asian}%).")
...
pull_tp_poverty_611 <- pull_poverty_age(c("Takoma", "6-11"), "pct_agepov")
pull_md_poverty_611 <- pull_poverty_age(c("^Maryland", "6-11"), "pct_agepov")
...
pull_md_poverty_6074 <- pull_poverty_age(c("^Maryland", "60-74"), "pct_agepov")
...
pull_asian_75_poverty_moe <- pull_race_age_poverty(c("Takoma", "75+", "asian"), "pct_moe")

pull_asian_6074_poverty <- pull_race_age_poverty(c("Takoma", "60-74", "asian"), "pct_age")

pull_white_6074_poverty <- pull_race_age_poverty(c("Takoma", "60-74", "white"), "pct_age")

pull_asian_6074_poverty_mont <- pull_race_age_poverty(c("Mont", "60-74", "asian"), "pct_age")

pull_asian_6074_poverty_md <- pull_race_age_poverty(c("^Maryland", "60-74", "asian"), "pct_age")
...
text_poverty_race_age <- glue("The large margin of error with smaller-samples makes it difficult to compare estimates by age and race. For instance, the poverty rate estimate for Asian residents 75 and over is {pull_asian_75_poverty}% but the margin of error is {pull_asian_75_poverty_moe}%, meaning that the actual value could be between 0% and 100%. **Still, some statistically-significant racial disparities emerge for specific age groups:**
...
- Poverty rates for Asian residents over 75 jump to {pull_asian_75_poverty}% (representing {pull_asian_75_poverty_tot} residents). However, due to the large margin of error, the difference isn't statistically significant compared to other groups. From age 60-74 though, Asian residents' poverty rate of {pull_asian_6074_poverty}% exceeds white residents' in that age range ({pull_white_6074_poverty}), as well as Asian residents in Montgomery County ({pull_asian_6074_poverty_mont}%) and Maryland ({pull_asian_6074_poverty_md}%).")
```
...
text_race_summary <- glue("**Takoma Park is a diverse city with a population of {pull_pop_tp %>% commafy}.** By race and ethnicity, {pull_white_tp_pct}% of residents are white (non-Hispanic), {pull_black_tp_pct}% Black, {pull_hisp_tp_pct}% Hispanic (of any race), and {pull_asian_tp_pct}% Asian.")
...
text_raceage <- glue("**Different races have different age-distributions compared to the city overall:**
                     ...
                     - White residents tend to be older compared to the rest of the city. There are a lower share of white residents aged 25-34 ({pull_white_2534_pct}%), and a higher share of residents aged 55-64 ({pull_white_5564_pct}%) and 65-74 ({pull_white_6574_pct}%).
                     - For Black residents, there are a lower share of residents 55-64 ({pull_black_5564_pct}%), and a higher share of residents aged 25-34 ({pull_black_2534_pct}%, although this difference is not statistically significant). The highest share of Black residents are aged 35-44 at {pull_black_3544_pct}%, but there is not a statistically-significant difference from the city's overall percent of aged 35-44 year old residents ({pull_overall_3544_pct}%). 
                     - For Hispanic residents, there are a higher share of residents aged 18-24 ({pull_hispanic_1824_pct}%), and a lower share of residents aged 55-64 ({pull_hispanic_5564_pct}%, although this difference is not statistically significant). 
                     - Asian residents and residents with a race of 'other' mostly follow the City's demographics, but there are a lower share of 0-9 Asian residents at {pull_asian_09_pct}%. For, residents with a race of 'other,' there are a higher share of residents between 18 and 24 ({pull_other_1824_pct}%).
...
A higher share of married Takoma Park households have children (`r pull_tp_married_child_pct`%) than Montgomery County (`r pull_mc_married_child_pct`%) or Maryland (`r pull_md_married_child_pct`%). A smaller share of single-woman households live with relatives in Takoma Park, at `r pull_tp_female_rel_pct`% compared to `r pull_mc_female_rel_pct`% in Montgomery County and `r pull_md_female_rel_pct`% in Maryland.

#### Age by sex

The largest share of female residents in the City are aged 40-49 at `r pull_female_4049_pct`% of female-residents and `r pull_female_4049_pctpop`% of the population, followed by people aged 30-39 (representing `r pull_female_3039_pct`% of the female-residents and `r pull_female_3039_pctpop`% of the City's population). The largest share of male residents are aged 0-9--representing `r pull_male_09_pct`% of males and `r pull_male_09_pctpop`% of the population--and 40-49, representing `r pull_male_4049_pct`% of males and `r pull_male_4049_pctpop`% of the population.

The biggest between-sexes differences by age are in people aged 30-49. There are `r pull_diff_3039_tot + pull_diff_4049_tot` more female-residents between 30 and 39 than male (representing `r pull_female_3039_pct + pull_female_4049_pct`% of female residents compared to `r pull_male_3039_pct+pull_male_4049_pct`% of male residents). 
...
Approximately `r pull_tp_rent_pct`% of Takoma Park's population are renters, higher than Maryland or Montgomery County.
...
**Takoma Park has `r pull_tp_unemploy_num` unemployed residents under 65,** for an unemployment rate of `r pull_tp_unemploy`%. 

`r text_raceunemploy_summary`

**Unemployment rates decline with age from a high of `r pull_tp_unemploy_2021`% at 20-21 to a low of 0 percent for people aged 60 to 61 and 65 to 69.** No differences between Takoma Park and Maryland or Montgomery County are statistically significant.
...
**Takoma Park has a similar poverty rate to Montgomery County and Maryland.** `r pull_tp_poverty`% of Takoma Park's population is below the poverty line (`r pull_tp_poverty_tot %>% commafy` residents), compared to `r pull_mont_poverty`% of Montgomery County. An additional `r pull_tp_below150-pull_tp_poverty`% (`r (pull_tp_below150_tot - pull_tp_poverty_tot) %>% commafy` residents) of Takoma Park's population earn between 100% and 149% of the poverty level (compared to `r pull_mont_below150-pull_mont_poverty`% for Montgomery County), and an additional `r pull_tp_below2-pull_tp_below150`% (`r (pull_tp_below2_tot - pull_tp_below150_tot) %>% commafy` residents) earn between 150% and 199% of the poverty line (compared to `r pull_mont_below2-pull_mont_below150`% in Montgomery County). Given [the outdated nature of US poverty measures](https://www.businessinsider.com/us-poverty-measure-misleading-outdated-leaves-out-millions-american-families-2020-7), many of these residents are economically vulnerable.
...
**Residents less than age 6 and between 12 and 17 had a statistically lower poverty rate than the City's overall rate of of `r pull_tp_poverty_overall`%, and residents 60-74 had a statistically higher poverty rate**. Children under 6 had a poverty rate of `r pull_tp_poverty_6`%, residents aged 12 to 17 had a poverty rate of `r pull_tp_poverty_1217`%, and residents aged 60-74 had a `r pull_tp_poverty_6074`% poverty rate. 

**Some age groups did have a statistically significant difference in poverty-rates compared to Montgomery County and Maryland.** Takoma Park residents aged 60 to 74 had higher poverty rates than Montgomery County and Maryland, at `r pull_tp_poverty_6074`% compared to `r pull_mont_poverty_6074`% and `r pull_md_poverty_6074` respectively. Residents aged 6-11 had a statistically lower poverty rate than Maryland at `r pull_tp_poverty_611`% compared to `r pull_md_poverty_611`%; and residents under 6 and aged 12-17 had a lower poverty rate than Montgomery County or Maryland at `r pull_tp_poverty_6`% compared to `r pull_mont_poverty_6`% and `r pull_md_poverty_6`% respectively for residents under 6, and `r pull_tp_poverty_1217`% compared to `r pull_mont_poverty_1217`% and `r pull_md_poverty_1217`% for residents aged 12-17.
...
Compared to the City's household poverty rate of `r pull_house_pov`%, poverty rates for female nonfamily households (`r pull_fem_pov_tp_pct`%) and male nonfamily households (`r pull_male_pov_tp_pct`%) are higher at a statistically significant level, and poverty rates for married couples are lower (`r pull_marr_pov_tp_pct`%). Poverty rates for female and male nonfamily households are also significantly higher than Montgomery County (`r pull_fem_pov_mont_pct`% and `r pull_male_pov_mont_pct`% respectively), and poverty rates for single-female family households (`r pull_femfam_pov_tp_pct`% in Takoma Park) are lower than Maryland (`r pull_femfam_pov_md`%).
...
**`r pull_tp_broadband`% of Takoma Park residents have access to broadband internet.** Of those without broadband access, `r pull_tp_noint`% have a computer but no internet, and `r pull_tp_nocomp`% have no computer.
...
**Compared to other age groups, 19-64 year old residents are most likely to not have any insurance.** `r pull_none_1934_pct`% of 19-34 year-old residents (`r pull_none_1934_tot` total) and `r pull_none_3564_pct`% of 35-64 year old residents (`r pull_none_3564_tot %>% tpfuncts::commafy()` total) have no insurance, followed by `r pull_none_under18_pct`% of under-19 residents (`r pull_none_under18_tot` total).
