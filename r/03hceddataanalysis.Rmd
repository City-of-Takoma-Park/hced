...
race_factor <- function(df, race_col = race){
  ...
  df %>%
    mutate({{race_col}} := factor({{race_col}}, race_vec, race_vec))
}
...
signif_compare <- function(df, 
  ...
  filter_df <- df %>%
    ...
    ungroup %>%
    filter(grepl(filter_val, {{ filter_col }}, ignore.case = T)) 
  ...
  filter_df <- filter_df %>%
    select({{join_col}}, {{ est_col }}, {{ moe_col }}) %>%
    distinct %>%
    rename(!!sym(est_filter) := {{ est_col }},
  ...
  df %>%
    left_join(filter_df) %>%
    mutate(!!sym(crit_filter) := signif_test({{ est_col }},
                                             !!sym(est_filter),
                                             {{ moe_col }},
                                             !!sym(moe_filter))[[1]],
           !!sym(signif_filter) := signif_test({{ est_col }},
                                               !!sym(est_filter),
                                               {{ moe_col }},
                                               !!sym(moe_filter))[[2]])
...
signif_overall <- function(processed_df,
  ...
  overall_df <- root_df %>%
    ...
    est_moe_derive(group_cols = group_cols[1:(length(group_cols)-1)], 
                   name_col = "pop_total") %>%
    est_moe_derive(group_cols = group_cols,
                   name_col = "overall") %>%
    select(group_cols, pop_total_est, pop_total_moe, overall_est, overall_moe) %>%
    distinct %>%
    derive_pct_est_moe("pct_overall",
                       "pop_total_est",
                       "pop_total_moe",
                       "overall_est",
                       "overall_moe")
  ...
  overall_df_join <- overall_df %>%
    select(group_cols, pct_overall, pct_moe) %>%
    rename(pct_moe_overall = pct_moe)
  
  # join and calculate significance
  processed_df <- processed_df %>%
    ungroup %>%
    left_join(overall_df_join) %>%
    mutate(crit_overall = signif_test(est1 = {{ est_col }}, 
                                      est2 = pct_overall, 
                                      moe1 = {{ moe_col }}, 
                                      moe2 = pct_moe_overall)[[1]],
           signif_overall = signif_test(est1 = {{ est_col }}, 
                                        est2 = pct_overall, 
                                        moe1 = {{ moe_col }}, 
  ...
  if (!is.null(bind_overall)){
    ...
    overall_df_bind <- overall_df %>%
      ungroup %>%
      mutate(!!sym(bind_overall) := "Overall") %>%
      select(group_cols, bind_overall, pct_overall, pct_moe) %>%
      rename({{ est_col }} := pct_overall,
             {{ moe_col }} := pct_moe)
...
# function to perform comparison with montgomery county and takoma park
signif_mont_tp <- function(df,
  ...
  df %>%
    signif_compare(filter_col = location,
                   filter_val = "mont",
                   join_col = join_col, 
                   est_col = {{ name_pct }}, 
                   moe_col = {{ moe_pct }}) %>%
...
process_df <- function(df,
  ...
  # 
  # else {
  #   if (!is.character(c("cats", "dogs"))){
  #     stop("error; supplied value to tot_derive is not a character vector")
  #   }
  #   
  #   # if totals already exist - supply those as character vector because lower moe
  #   processed_df <- processed_df %>%
  #     rename(tot_est = tot_derive[1],
  #            tot_moe = tot_derive[2])
  #   
  # }
  # 
  ...
  processed_df <- processed_df %>%
    select(group_cols, tot_est, tot_moe, name_est, name_moe) %>%
    distinct() %>%
    derive_pct_est_moe(name_pct,
                       "tot_est",
                       "tot_moe",
                       name_est,
                       name_moe) %>%
    signif_overall(root_df = root_df, 
                   group_cols = overall_cols, 
                   est_col = !!sym(name_pct), 
                   moe_col = pct_moe,
                   bind_overall = bind_overall) %>%
    signif_mont_tp(join_col = group_cols[-1], 
                   name_pct = !!sym(name_pct)) %>%
    signif_checker(signif_cols = signif_cols) %>%
    num_formatter() 
...
# graph data
num_formatter <- function(df, arrange_col = NULL){
  
  if (!is.null(arrange_col)){
    return(df %>%
             mutate(across(where(is.numeric), .fns = ~ round(.x, 2))) %>%
             arrange(desc(!!sym(arrange_col))))
  }
...
name_formatter <- function(df){
  df %>%
    mutate(location = case_when(grepl("Takoma", name) ~ "Takoma Park",
...
# wrapper to apply name and number formatter
name_num_formatter <- function(df, arrange_col = NULL){
  df %>%
    num_formatter(arrange_col = arrange_col) %>%
    name_formatter()
}

recode_tenure <- function(df, tenure_col){
  df %>%
    mutate({{ tenure_col }} := str_extract({{ tenure_col }}, "^.*occupied"))
}
...
# Takoma Park Data Explorer {- .tabset}

***About***: This is a webpage to explore selected Census data on Takoma Park from the 2015-2019 American Community Survey. Each heading tab opens to a "Summary" sub-tab; click on other sub-tabs to see data-visualizations and detailed explanations for each type of data. All graphics are interactive; you can click and double-click on legend items to hide or show them, and mousing over graphics will show data-values. Race and ethnicity data are consolidated in the "Race and Ethnicity" tab, and repeat in each subject-area heading tab (e.g., race/ethnicity housing data can be found in the "Housing" sub-tab of the "Race and Ethnicity" heading-tab, as well as in the "Housing" heading-tab).
...
# function to identify if data statistically significant
signif_checker <- function(data, 
  ...
  # rotate through
   walk2(signif_cols, names(signif_cols), ~ {
     data <<- data %>%
       mutate(signif_check = paste0(signif_check,
                                    case_when(is.na(!!sym(.x)) ~ "",
                                              TRUE ~ paste0(", ", .y))))
   })
   
   data %>%
     mutate(signif_check = ifelse(signif_check == "", 
                                  "None", 
                                  gsub("^, ", "", signif_check)))
...
age_recode <- function(df, age_col, regroup = "decennial") {
  ...
  df <- df %>%
    mutate(age_new = age_sub({{ age_col }}))
  
  # pull vector
  age_vec_order <- pull(df, age_new) %>%
    unique()
  ...
  df <- df %>%
    mutate(age_new = factor(age_new, age_vec_order, age_vec_order))
  ...
  if (regroup == "decennial"){
    ...
    process_age_group <- function(string){
      ...
      case_when(string %in% c(less_age, single_dig) ~ age_grp_vec[1],
    ...
    df <- df %>%
      mutate(age_group = process_age_group(age_new),
...
birth_recode <- function(df, birth_col){
  ...
  birth_change <- function(string){
    case_when(grepl("state of residence", string) ~ birth_vec[1],
  ...
  df <- df %>%
    mutate(birth_new = birth_change({{ birth_col }}),
...
asian_country_recode <- function(df, ethnicity_col){
  ...
  change_ethnicity <- function(string){
    case_when(grepl("Indian", string) ~ "Indian",
  ...
  df %>%
    mutate(new_ethnicity = change_ethnicity( {{ ethnicity_col }}))
}
...
ancestry_recode <- function(df, ancestry_col){
  ...
  ancestry_change <- function(string){
    case_when(grepl("Arab:", string) ~ "Arab",
  ...
  df %>%
    mutate(ancestry_new = ancestry_change({{ ancestry_col }}))
...
plot_ancestry_sub <- plot_ly(ancestry_sub_tp_process,
                             ...
                             hovertext = ~ glue("Percent: {pct_country}
                           Margin of error: {moe}"),
                           color = ~ country) %>%
...
health_insur_recode <- function(df, num, type){
  ...
  string_recode <- function(num_plans, type_coverage){
    multi_plans <- case_when(grepl("one", num_plans) ~ F,
    ...
    case_when(multi_plans ~ multi_funct(type_cov = type_coverage),
  ...
  df <- df %>%
    mutate(health_new := string_recode({{ num }}, {{ type }}),
           health_new = factor(health_new, health_vec, health_vec))
  
  if (any(is.na(df[["health_new"]]))){
    
    miss <- df %>%
      filter(is.na(health_new)) %>%
      pull({{type}}) %>%
      unique
    # browser()
    
    stop(glue("Error; missing values in returned vector; types coverage producing missing values are {paste(miss, collapse = ',')}"))
  }
...
text_race_educ <- glue("**Significant racial disparities exist in educational attainment in Takoma Park, especially for Black residents, Hispanic residents, and residents with a racial categorization of 'other.'**
                               ...
                               - {pull_tp_black_lesshs + pull_tp_black_hs}% of the city's Black population have a high school degree or did not complete high school ({pull_tp_black_lesshs_tot + pull_tp_black_hs_tot} total), compared to {pull_tp_white_hs + pull_tp_white_lesshs}% of the city's white population ({pull_tp_white_hs_tot + pull_tp_white_lesshs_tot} total). 
                               - {pull_tp_other_lesshs}% of residents with a race of 'other' do not have a high school diploma ({pull_tp_other_lesshs_tot} total); the low number of residents with a race of 'other' and the margin of error of {pull_other_race_educ_moe}% (meaning we have 90% confidence the percentage of residents without a high school degree is between {pull_other_race_educ_pctlower} and {pull_other_race_educ_pctupper}%) suggest some caution in interpreting this result, but even the lower bound of {pull_other_race_educ_pctlower}% or residents without a high school diploma would exceed all other groups except for Hispanic residents.
...
text_computer_race_summary <- glue("**White and Asian residents of Takoma Park have the greatest access to broadband internet while residents with a race of 'other,' Hispanic, and Black residents have the least.** {pull_tp_asian_broad}% of the City's Asian residents and {pull_tp_white_broad}% of its white residents ({pull_tp_white_broad_tot} total) have access to broadband internet, compared to {pull_tp_other_broad}% of residents with a race of 'other'  {pull_tp_hispanic_broad}% of Hispanic residents, and {pull_tp_black_broad}% of Black residents. Rates of access for Asian Takoma Park residents exceed Asian residents of Montgomery County and Maryland, rates for white residents exceed Maryland, and rates for Hispanic/'other' residents fall-below Maryland and Montgomery County.")
...
health_race <- process_df(health_insur_race_age_process, 
...
plot_health_race_noinsur <- plot_ly(health_race_noinsur %>% race_factor(),
        hovertext = ~ glue("Estimate: {health_est}
...
# plot_age_pct_tenure <- plot_ly(tp_tenure_age,
#         x = ~ tenure,
#         y = ~ estimate,
#         color = ~ age_new,
#         name = ~ age_new,
#         text = ~ glue("{age_new}: {pct_age}%"),
#         hovertext = ~ glue("Percent: {pct_age}%
#                            Significant differences: {signif_check_compage}
#                            Percent age group of city population: {pct_age_pop}%"),
#         textfont = list(color = "black"),
#         textposition = "inside") %>%
#   layout(barmode = "stack",
#          xaxis = list(title = "Tenure"),
#          yaxis = list(title = "Householders"),
#          title = "Total householders by age and tenure")
...
# test <- data_binder("burden_renters_processed") %>%
...
cost_burden_recode <- function(df, cost_col){
  ...
  unique_costs <- pull(df, {{ cost_col }}) %>%
    unique
  
  factor_levels <- c(case_when(any(grepl("Less than 20", unique_costs)) ~ c("< 20%"),
  ...
  df %>%
    mutate({{ cost_col }} := string_recode({{ cost_col }})) %>%
    mutate({{ cost_col }} := factor({{ cost_col }}, factor_levels, factor_levels))
...
incpov_recode <- function(df, var){
  ...
  df %>%
    mutate(incpov_new = factor({{ var }}, incpov_vec, incpov_vec))
...
unemploy_overall_processed <- process_df(unemploy_gender_age_root, 
...
# process data
unemploy_age_processed <- process_df(unemploy_root, 
...
# function to join older age groups for greater statistical
old_recode <- function(df, age_col){
  age_old <- function(string){
    case_when(grepl("(75)|(85)", string) ~ "75 years and over",
              T ~ string)
  }
  
  df %>%
    mutate({{ age_col }} := age_old({{ age_col }}))
}
...
poverty_age_processed <- process_df(poverty_age_root, 
...
text_raceunemploy <- glue("By race, statistically-significant differences in unemployment rates appear between Black residents and white residents, and between white residents and the city's overall unemployment rate of {pull_tp_unemploy}%. Black residents under age 65 experience the highest unemployment levels at {pull_tp_black_unemploy}% (representing {pull_tp_black_unemploy_tot} total residents) compared to {pull_tp_white_unemploy}% for white residents (representing {pull_tp_white_unemploy_tot} residents). For other races, the low sample-sizes result in a high margin of error, meaning it is difficult to confidently assess whether differences in unemployment estimates actually exist or appear because of the particular group of residents the Census surveyed. For instance, Hispanic residents have the highest unemployment-rate estimate at {pull_tp_hispanic_unemploy}% but the margin of error is {pull_tp_hispanic_unemploy_moe}%, meaning that we are 90% confident that the actual estimate falls between {pull_tp_hispanic_unemploy - pull_tp_hispanic_unemploy_moe}% and {pull_tp_hispanic_unemploy + pull_tp_hispanic_unemploy_moe}%.")
...
text_poverty_race_age <- glue("The large margin of error with smaller-samples makes it difficult to compare estimates by age and race. For instance, the poverty rate estimate for residents with a race of 'other' less than 6 is {pull_other_6_poverty}% but the margin of error is {pull_other_6_poverty_moe}%, meaning we have 90% confidence that the actual value is between 0% and {pull_other_6_poverty +pull_other_6_poverty_moe}%. Nonetheless, some statistically-significant racial disparities still emerge for specific age groups:
...
text_race_summary <- glue("**Takoma Park is a majority-minority city with a population of {pull_pop_tp %>% commafy}.** By race, {pull_white_tp_pct}% of residents are white (non-Hispanic), {pull_black_tp_pct}% Black, {pull_hisp_tp_pct}% Hispanic (of any race), and {pull_asian_tp_pct}% Asian.")

text_ancestry_summary <- glue("**The largest reported non-European country of ancestry for residents is Ethiopia**, representing {pull_ethiopian_pct}% of the {pull_ssa %>% commafy} residents the Census reports as having sub-Saharan African ancestry. **The three largest origin-groups of people with a reported race of Asian are Indian, Chinese, and Vietnamese** (representing {pull_indian}, {pull_chinese}, and {pull_vietnamese} residents respectively).")
...
**Takoma Park is a majority-minority city with a population of `r pull_pop_tp %>% commafy`.** The city's three largest racial/ethnic groups are non-Hispanic white (`r pull_white_tp_pct`%), Black (`r pull_black_tp_pct`%), and Hispanic (`r pull_hisp_tp_pct`%). Almost a third of residents were born in another county; of those born in another country, `r pull_tp_africa`% were born in Africa and `r pull_tp_latin`% in Latin America. A little more than a third of residents speak another language at home, the most common being Spanish at `r pull_tp_spanish`% of residents.
...
- *Education*: A higher share of the city's residents have a graduate degree than Mongtgomery County or Maryland (`r pull_tp_graduate`%) and a higher share have a graduate or bachelor's degree than Maryland (`r pull_tp_bachelor + pull_tp_graduate`%), but a lower share completed high school (`r pull_tp_lesshs`%) than Montgomery County or Maryland. There are substantial racial disparities in educational attainment; `r pull_tp_hisp_lesshs`% of the city's Hispanic residents do not have a high school diploma compared to `r pull_tp_white_lesshs`% of the city's white residents, and `r pull_tp_black_lesshs + pull_tp_black_hs`% of the city's Black population have a high school degree or did not complete high school compared to `r pull_tp_white_hs + pull_tp_white_lesshs`% of the city's white residents. 
...
text_tenure_costburdenincome <- glue("Breaking down cost burden by income reveals that while low-income homeowners experience are most likely to experience extreme cost-burdens (spending more than 50% of their income on rent), renters overall are much more likely to experience cost burden because renters generally earn much less than homeowners. 
...
pull_creator_test <- function(df_result, filter_col, default_pull = NULL, ...){
...
pull_race_age_tp_test <- pull_creator_test(race_age_tp, c("race", "age_group"))
...
plot_race_sex <- plot_ly(race_gender_bound_tp %>%
                         ...
                         color = ~ gender,
                         name = ~ gender,
                         hovertext = ~ glue("Total: {gender_tot}
                           Margin of error: {pct_moe %>% round(1)}
...
text_map_race <- glue("The map below shows the distribution of different races across the city. White residents are distributed throughout the city, but represent a greater share of residents in the South, West, and center. Black residents are concentrated in the North and East of the city, and Hispanic residents in the North and Northeast of the city. Asian residents are concentrated in the East of the city, and residents with a race of 'other' in the Northeast.")

```

`r text_map_race`

```{r map race, include=FALSE}

bg_shapes_racedata_tp <- st_read("./data/output_data/shapefiles/bg_shapes_racedata_tp.geojson")
  ...
  map(seq(nrow(df_as_data)), function(i){
    return(
      glue::glue("{df_as_data[i, 'name_e']}<p></p>
        Total white (non-Hispanic) residents: {df_as_data[i, 'hispr_nothisp_white']}<p></p>
        Total Black residents: {df_as_data[i, 'race_black']}<p></p>
        Total Hispanic residents: {df_as_data[i, 'hispr_hisplat']}<p></p>
        Total Asian residents: {df_as_data[i, 'race_asian']}<p></p>
        Total 'other' residents: {df_as_data[i, 'race_other']}<p></p>
        Percent white (non-Hispanic) residents: {df_as_data[i, 'hispr_nothisp_white_pct']}<p></p>
        Percent Black residents: {df_as_data[i, 'race_black_pct']}<p></p>
        Percent Hispanic residents: {df_as_data[i, 'hispr_hisplat_pct']}<p></p>
        Percent Asian residents: {df_as_data[i, 'race_asian_pct']}<p></p>
        Percent 'other' residents: {df_as_data[i, 'race_other_pct']}")
    )
  })
}

labels_funct(bg_shapes_racedata_tp %>% st_drop_geometry())


map_funct_racedata <- function(basemap = bg_shapes_racedata_tp, 
  ...
  map <- leaflet(basemap) %>%
    ...
    addpoly_legend(as_data_file, 
                   pal_numeric("hispr_nothisp_white", 
                               df = as_data_file, 
                               "Reds"), 
                   "hispr_nothisp_white",
                   group_select = "White (non-Hispanic) residents: total", 
                   "Total", 
                   labels_select = label_result) %>%
    addpoly_legend(as_data_file, 
                   pal_numeric("race_black", 
                               df = as_data_file, 
                               "Blues"), 
                   "race_black", 
                   group_select = "Black residents: total", 
                   "Total", 
                   labels_select = label_result) %>%
    addpoly_legend(as_data_file, 
                   pal_numeric("hispr_hisplat", 
                               df = as_data_file, 
                               "Greens"), 
                   "hispr_hisplat", 
                   group_select = "Hispanic residents: total", 
                   "Total", 
                   labels_select = label_result) %>%
    addpoly_legend(as_data_file, 
                   pal_numeric("race_asian", 
                               df = as_data_file, 
                               "Oranges"), 
                   "race_asian", 
                   group_select = "Asian residents: total",
                   "Total", 
                   labels_select = label_result) %>%
    addpoly_legend(as_data_file, 
                   pal_numeric("race_other", 
                               df = as_data_file, 
                               "Greys"), 
                   "race_other", 
                   group_select = "'Other' residents: total", 
                   "Total", 
                   labels_select = label_result) %>%
    addpoly_legend(as_data_file, 
                   pal_numeric("hispr_nothisp_white_pct", 
                               df = as_data_file, 
                               "Reds"), 
                   "hispr_nothisp_white_pct",
                   group_select = "White (non-Hispanic) residents: percent", 
                   "Percent", 
                   labels_select = label_result) %>%
    addpoly_legend(as_data_file, 
                   pal_numeric("race_black_pct", 
                               df = as_data_file, 
                               "Blues"), 
                   "race_black_pct", 
                   group_select = "Black residents: percent", 
                   "Percent", 
                   labels_select = label_result) %>%
    addpoly_legend(as_data_file, 
                   pal_numeric("hispr_hisplat_pct", 
                               df = as_data_file, 
                               "Greens"),
                   "hispr_hisplat_pct", 
                   group_select = "Hispanic residents: percent", 
                   "Percent", 
                   labels_select = label_result) %>%
    addpoly_legend(as_data_file, 
                   pal_numeric("race_asian_pct", 
                               df = as_data_file, 
                               "Greys"), 
                   "race_asian_pct", 
                   group_select = "Asian residents: percent", 
                   "Percent", 
                   labels_select = label_result) %>%
    addpoly_legend(as_data_file, 
                   pal_numeric("race_other_pct", 
                               df = as_data_file, 
                               "Oranges"), 
                   "race_other_pct", 
                   group_select = "'Other' residents: percent", 
                   "Percent", 
                   labels_select = label_result) %>%
...
map_produce <- map_funct_racedata() %>%
  add_wards()
...
plot_ly(sex %>%
        ...
        hole = 0.4,
        type = "pie") %>%
  layout(title = "Takoma Park by sex"
  )
...
text_disab_race <- glue("There appear to be some differences in disability-rates by race, but all results fall within each other's margin of error (meaning we cannot conclude there is an actual difference at a level of 90% confidence). 
...
2) [Housing cost burden by tenure]

3) [Housing cost burden by tenure and income]
...
*Note: Race/ethnicity categories are only included in the table above if at least 30 workers in that category were unemployed in Takoma Park.*
...
plot_ly(data = tenure_data,
        ...
        name = ~ location,
        color = ~ location) %>%
  layout(title = "Households by tenure",
         xaxis = list(title = "Tenure"),
         yaxis = list(title = "Percent of households"))
         # barmode = "stack")
...
**Compared to Montgomery County and Maryland, Takoma Park has a larger share of people with the highest levels of educational attainment and minimal educational attainment.** `r pull_tp_bachelor + pull_tp_graduate`% of the city's population has a graduate or bachelor's degree--exceeding Maryland--and `r pull_tp_graduate`% have a graduate degree, exceeding Montgomery County and Maryland. At the other end, `r pull_tp_lesshs`% of residents did not complete high school--exceeding Montgomery County and Maryland--and `r pull_tp_lesshs + pull_tp_hs`% of residents' highest level of educational attainment was a high school degree or less, exceeding Montgomery County.
...
## Methodology and update-list {-}

### Methodology

This document visualizes selected data from the 2015-2019 American Community Survey. Data is downloaded from the Census using R's tidycensus package. Block-group data in the racial demographics map as well as the shapefile come from NHGIS, cited below. Data will be updated periodically (potentially with additional visualizations), and with 2016-2020 data once the newest version of the American Community Survey is released. Updates are described below.

Text descriptions of the data generally focus on statistically significant differences at a minimum 90% significance level. For a place as small as Takoma Park it can be difficult to assess whether differences in the data are 'real,' or due to chance or the particular sample of people that the Census surveyed. This is especially true for analyses involving smaller subgroups or greater levels of disaggregation. For example, Takoma Park has a very small population of Native Hawaiians and Pacific Islanders, and populations of most groups become small when you break out data by race, age, and sex. For a difference to be statistically significant at a 90% level means that we would expect the difference to appear in the data due to randomness or sampling no more than 10% of the time. The hover text of many interactive visualizations in the explorer describe whether there are statistically significant differences between the selected group and other groups, the level of confidence in a given difference, and/or the margin of error of the estimate.

For instance, in the plot below of internet access for residents 65 and over, hovering over Takoma Park's broadband internet results shows "Margin of error: 7.19%" and "Significant Differences: Overall, MC." The margin of error means that we are 90% confident that the actual level of broadband internet access of people 65 and over--not just the 'observed' estimate, which is what we see in the survey data--is between `r 75.97 - 7.19`% and `r 75.97 + 7.19`%. The significant differences results means that we have at least 90% confidence that there are actual-differences--and not just observed differences--between broadband access for Takoma Park residents 65+ and broadband access for all residents in the city ("Overall"), and between broadband access for 65+ residents of Takoma Park and 65+ residents of Montgomery County ("MC"). We don't see "MD" appear in the list of significant differences because we cannot have confidence at a 90% significance level that Takoma Park's 65+ broadband-access rate of 75.97% is actually-different than Maryland's 65+ access rate of 77.12%. As a basic illustration of this, the margin of error suggests that Takoma Park's rate could be either higher or lower due to randomness or sampling.

```{r demo subplot}
plot_gen_agecomp("65+") %>%
  layout(title = "Internet access by for residents 65 and over",
         yaxis = list(title = "Percent"))
